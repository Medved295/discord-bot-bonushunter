...
# 🔥 ИСПРАВЛЕННЫЙ ЗАПУСК С ПРОВЕРКОЙ ТОКЕНА
...
if token:
    if token.startswith('MT') or token.startswith('OT'):
        print("✅ Токен выглядит корректно")
        bot.run(token)
    else:
        print("❌ Токен выглядит некорректно")
        print("🔧 Убедитесь что в Render переменная DISCORD_TOKEN содержит правильный токен")
else:
    print("❌ ТОКЕН НЕ НАЙДЕН!")
    print("📋 Инструкция по настройке:")
    print("1. Зайдите в ваш сервис на Render")
    print("2. Нажмите 'Environment' в левом меню")
